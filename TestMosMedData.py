...
def compress(nii_filepath: str, bin_filepath: str, lossless_net: Network,
    ...
    data = np.array(data.dataobj).astype(np.int32)
...
class Tester:
    def __init__(self) -> None:
        ...
        self.load_weights()
        self.mosmed_files = [os.path.join(self.args.mosmed_root, nii_filename)
                             for nii_filename in os.listdir(self.args.mosmed_root) if os.path.splitext(nii_filename)[-1] == '.nii']

    @torch.no_grad()
    def test(self) -> None:
        os.makedirs(self.args.save_directory, exist_ok=True)
        record = {}
        for nii_filepath in self.mosmed_files:
            ...
            ori_slices = np.array(ori_slices.dataobj).astype(np.int32)
        ...
        bpp_average = sum([record[k]['bpp'] for k in record.keys()]) / len(self.mosmed_files)
        lossy_bpp_average = sum([record[k]['lossy_bpp'] for k in record.keys()]) / len(self.mosmed_files)
        lossless_bpp_average = sum([record[k]['lossless_bpp'] for k in record.keys()]) / len(self.mosmed_files)
        lossy_enc_runtime_avg = sum([record[k]['lossy_enc_runtime'] for k in record.keys()]) / len(self.mosmed_files)
        lossless_enc_runtime_avg = sum([record[k]['lossless_enc_runtime'] for k in record.keys()]) / len(self.mosmed_files)
        lossy_dec_runtime_avg = sum([record[k]['lossy_dec_runtime'] for k in record.keys()]) / len(self.mosmed_files)
        lossless_dec_runtime_avg = sum([record[k]['lossless_dec_runtime'] for k in record.keys()]) / len(self.mosmed_files)
    ...
    def parse_args():
        ...
        parser.add_argument('--mosmed_root', type=str, help='path of folder of MRNet dataset')
